...
const actionConfig: Record<ActivityAction, { icon: React.ReactNode; color: string; bg: string; category: 'toll-gates' | 'approvals' }> = {
  ...
  gate_removed:      { icon: <ShieldOff size={14} />,     color: 'text-red-600',     bg: 'bg-red-50',     category: 'toll-gates' },
  gate_renamed:      { icon: <Edit3 size={14} />,         color: 'text-indigo-600',  bg: 'bg-indigo-50',  category: 'toll-gates' },
  criterion_added:   { icon: <Plus size={14} />,          color: 'text-teal-600',    bg: 'bg-teal-50',    category: 'toll-gates' },
  criterion_removed: { icon: <Trash2 size={14} />,        color: 'text-red-600',     bg: 'bg-red-50',     category: 'toll-gates' },
  criterion_checked: { icon: <CheckCircle2 size={14} />,  color: 'text-emerald-600', bg: 'bg-emerald-50', category: 'approvals' },
  criterion_revoked: { icon: <RotateCcw size={14} />,     color: 'text-amber-600',   bg: 'bg-amber-50',   category: 'approvals' },
  gate_go:           { icon: <ThumbsUp size={14} />,      color: 'text-emerald-600', bg: 'bg-emerald-50', category: 'approvals' },
  gate_no_go:        { icon: <ThumbsDown size={14} />,    color: 'text-red-600',     bg: 'bg-red-50',     category: 'approvals' },
};
...
export function ActivityPanel({ entries, onClose }: ActivityPanelProps) {
  ...
  return (
    <div className="w-[400px] max-w-[90vw] max-h-[calc(100vh-48px)] bg-white border border-grey-080 rounded-2xl flex flex-col shrink-0 shadow-xl shadow-black/8 overflow-hidden">
      {/* Header */}
      <div className="px-5 pt-4 pb-3 border-b border-grey-080">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-grey-900">Activity</h2>
              <p className="text-[11px] text-grey-400">{entries.length} total events</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          {FILTER_TABS.map(tab => (
            ...
            >
              ...
              <span className={`text-[10px] px-1.5 py-px rounded-full ${
                filter === tab.key ? 'bg-white/20 text-white' : 'bg-grey-100 text-grey-400'
              }`}>
                {badgeCounts[tab.key]}
              </span>
            </button>
      ...
      {Object.keys(gateStats).length > 0 && (
        <div className="px-5 py-2.5 border-b border-grey-080 bg-grey-050/50">
          <div className="flex items-center gap-1.5 mb-2">
            <Filter size={11} className="text-grey-400" />
            <span className="text-[10px] font-semibold text-grey-400 uppercase tracking-wider">Per Gate</span>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-0.5">
            {Object.entries(gateStats).map(([gate, stats]) => (
              <div key={gate} className="shrink-0 bg-white rounded-lg px-3 py-1.5 border border-grey-080 min-w-[110px]">
                <div className="text-[11px] font-semibold text-grey-700 truncate">{gate}</div>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[10px] text-emerald-600">{stats.approvals} approved</span>
                  {stats.decisions > 0 && (
                    <span className="text-[10px] text-primary-500">{stats.decisions} decision{stats.decisions !== 1 ? 's' : ''}</span>
                  )}
                </div>
              </div>
            ))}
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    ...
                    {entry.gateName && (
                      <div className="flex items-center gap-1 mt-1">
                        <span className="text-[10px] text-primary-500 bg-primary-100 px-1.5 py-0.5 rounded font-medium">{entry.gateName}</span>
                      </div>
                    )}
                  </div>
                </div>
