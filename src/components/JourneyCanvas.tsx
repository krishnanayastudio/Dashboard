...
function BlockCard({ block }: { block: Block }) {
  ...
  return (
    <div className="relative w-[288px] h-[230px] rounded-3xl border border-gray-100 shadow-[0px_1px_2px_rgba(0,0,0,0.07),0px_2px_6px_rgba(0,0,0,0.04)] overflow-hidden bg-white shrink-0 hover:shadow-lg transition-shadow cursor-pointer">
      ...
      {hasApproval && (
        <div className="absolute bottom-[10px] left-[10px] right-[10px]">
          <div className="flex items-center gap-2 bg-white rounded-lg shadow-[0px_1px_3px_rgba(0,0,0,0.1)] px-2.5 py-1.5">
            ...
            <span className={`text-xs font-medium ${isApproved ? 'text-green-500' : 'text-gray-800'}`}>
              ...
              <div className="flex items-center ml-auto">
                <div className="flex -space-x-1.5">
                  {block.approval!.assignees.slice(0, 1).map((name, i) => (
                    <div key={i} className="w-5 h-5 rounded-full bg-gradient-to-br from-purple-300 to-purple-500 border-2 border-white flex items-center justify-center">
                      <span className="text-[8px] font-bold text-white">{name.charAt(0)}</span>
                    </div>
                  ))}
                </div>
                {block.approval!.assignees.length > 1 && (
                  <span className="text-[10px] text-gray-700 ml-1 font-medium">
                    +{block.approval!.assignees.length - 1}
                  </span>
                )}
              </div>
            ...
            {block.approval!.dueDate && (
              <span className="text-[10px] text-gray-600 font-medium whitespace-nowrap ml-1">
                {block.approval!.dueDate}
              </span>
            )}
          </div>
...
function BlockGroupCard({ group }: { group: BlockGroup }) {
  ...
  return (
    <div className="bg-gray-50 rounded-3xl overflow-hidden shrink-0 w-[304px]">
